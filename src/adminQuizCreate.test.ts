...
describe('POST /v1/admin/quiz', () => {
  test('Token is non-existent', () => {
    const res = request('POST', SERVER_URL + '/v1/admin/quiz', 
      { json: {token: '0', name: 'Betty boop quiz', 
        description: 'Quiz for Betty boop'}, timeout: TIMEOUT_MS });
  ...
  describe('After creating tokens', () => {
    ...
    beforeEach(() => {
      const res = request('POST', SERVER_URL + '/v1/admin/auth/register', 
        { json: {email: 'bettyBoop@gmail.com', password: 'helloWorld1',
           nameFirst: 'Betty', nameLast: 'Boop'}, timeout: TIMEOUT_MS });
      token = JSON.parse(res.body.toString());
    });

    test('Token is not a valid token', () => {
      const res1 = request('POST', SERVER_URL + '/v1/admin/quiz', 
        { json: {token: token.token+'1', name: 'Betty boop quiz', 
          description: 'Quiz for Betty boop'}, timeout: TIMEOUT_MS });
    ...
    test('name already used for another quiz', () => {
      request('POST', SERVER_URL + '/v1/admin/quiz', { json: {token: token.token, 
        name: 'Betty boop quiz', description: 'Quiz for Betty boop'}, 
        timeout: TIMEOUT_MS });
      const res1 = request('POST', SERVER_URL + '/v1/admin/quiz', 
        { json: {token: token.token, name: 'Betty boop quiz', 
          description: 'Quiz for Betty boop'}, timeout: TIMEOUT_MS });
      expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
      expect(res1.statusCode).toStrictEqual(400);
    });
    ...
    test('Correctly created a quiz with empty description', () => {
      const res1 = request('POST', SERVER_URL + '/v1/admin/quiz', 
        { json: {token: token.token, name: 'Betty boop quiz', 
          description: ''}, timeout: TIMEOUT_MS });
      expect(JSON.parse(res1.body.toString())).toStrictEqual(
        {
          quizId: expect.any(Number),
        }
