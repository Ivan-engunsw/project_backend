...
describe('PUT /v1/admin/quiz/:quizId/question/:questionId/move', () => {
  test('Token is non-existent', () => {
    const res = request('PUT', SERVER_URL + '/v1/admin/quiz/0/question/0/move', { json: { token: '0', newPosition: 1 }, timeout: TIMEOUT_MS });
  ...
  describe('After creating tokens', () => {
    ...
    test('Token is not a valid user', () => {
      const res1 = request('PUT', SERVER_URL + '/v1/admin/quiz/0/question/0/move',
        { json: { token: token.token + '1', newPosition: 1 }, timeout: TIMEOUT_MS });
    ...
    describe('After creating quizzes', () => {
      ...
      test('QuizId does not exist', () => {
        const res1 = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId + 1}/question/0/move`,
      ...
      test('User is not an owner of the quiz', () => {
        ...
        const res1 = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId}/question/0/move`,
          { json: { token: token2.token, newPosition: 1 }, timeout: TIMEOUT_MS });
        expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
        expect(res1.statusCode).toStrictEqual(403);
      });
      ...
      describe('After creating questions', () => {
        ...
        test('QuestionId is not valid', () => {
          const res1 = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId}/question/${questionId.questionId + 1}/move`,
            { json: { token: token.token, newPosition: 1 }, timeout: TIMEOUT_MS });
          expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
        ...
        test('NewPosition is greater than no of questions', () => {
          const res1 = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId}/question/${questionId.questionId}/move`,
            { json: { token: token.token, newPosition: 2 }, timeout: TIMEOUT_MS });
          expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
          expect(res1.statusCode).toStrictEqual(400);
        });
        ...
        test('NewPosition is the position of the current question', () => {
          const res1 = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId}/question/${questionId.questionId}/move`,
            { json: { token: token.token, newPosition: 0 }, timeout: TIMEOUT_MS });
          expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
          expect(res1.statusCode).toStrictEqual(400);
        });
