...
describe('POST /v1/admin/quiz/:quizid/transfer', () => {
  ...
  describe('error testing', () => {
    test('returns an error for an invalid target email', () => {
      const res = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}/transfer`, { json: { token: token.token, userEmail: 'notanemail@unsw.com' }, timeout: TIMEOUT_MS });
      expect(JSON.parse(res.body.toString())).toStrictEqual(ERROR);
      expect(res.statusCode).toStrictEqual(400);
    });

    test('returns an error for the current user\'s email', () => {
      const res = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}/transfer`, { json: { token: token.token, userEmail: INPUT_USER_1.email }, timeout: TIMEOUT_MS });
      expect(JSON.parse(res.body.toString())).toStrictEqual(ERROR);
      expect(res.statusCode).toStrictEqual(400);
    });

    test('returns an error for a quiz name already in use by the target', () => {
      request('POST', SERVER_URL + '/v1/admin/quiz', { json: { token: token2.token, name: 'Quiz1', description: 'Norman\'s quiz' }, timeout: TIMEOUT_MS });
    ...
    test('returns an error for a quiz not owned by this user', () => {
      const resQuiz2 = request('POST', SERVER_URL + '/v1/admin/quiz', { json: { token: token2.token, name: 'Quiz1', description: 'Norman\'s quiz' }, timeout: TIMEOUT_MS });
      const quiz2 = JSON.parse(resQuiz2.body.toString());
      const res = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz2.quizId}/transfer`, { json: { token: token.token, userEmail: INPUT_USER_2.email }, timeout: TIMEOUT_MS });
      expect(JSON.parse(res.body.toString())).toStrictEqual(ERROR);
      expect(res.statusCode).toStrictEqual(403);
    });

    test('returns an error for an invalid authUserId', () => {
      const res = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}/transfer`, { json: { token: token.token + 1, userEmail: INPUT_USER_2.email }, timeout: TIMEOUT_MS });
      expect(JSON.parse(res.body.toString())).toStrictEqual(ERROR);
      expect(res.statusCode).toStrictEqual(401);
    });

    test('returns an error for an invalid quizId', () => {
      const res = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz.quizId + 1}/transfer`, { json: { token: token.token, userEmail: INPUT_USER_2.email }, timeout: TIMEOUT_MS });
      expect(JSON.parse(res.body.toString())).toStrictEqual(ERROR);
      expect(res.statusCode).toStrictEqual(403);
    });
  });
  ...
  describe('functionality testing', () => {
    ...
    test('quiz is successfully transferred to target', () => {
      request('POST', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}/transfer`, { json: { token: token.token, userEmail: INPUT_USER_2.email }, timeout: TIMEOUT_MS });
