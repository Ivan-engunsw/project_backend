...
describe('POST /v1/admin/quiz/:quizId/question/:questionId/duplicate', () => {
  test('Token is non-existent', () => {
    const res = request('POST', SERVER_URL + '/v1/admin/quiz/0/question/0/duplicate',
      { json: { token: '0' }, timeout: TIMEOUT_MS });
  ...
  describe('After creating tokens', () => {
    ...
    test('Token is not a valid user', () => {
      const res1 = request('POST', SERVER_URL + '/v1/admin/quiz/0/question/0/duplicate',
        { json: { token: token.token + '1' }, timeout: TIMEOUT_MS });
    ...
    describe('After creating quizzes', () => {
      ...
      test('QuizId does not exist', () => {
        const res1 = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId + 1}/question/0/duplicate`,
          { json: { token: token.token }, timeout: TIMEOUT_MS });
        expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
      ...
      test('User is not an owner of the quiz', () => {
        const resUser2 = request('POST', SERVER_URL + '/v1/admin/auth/register',
          { json: { email: 'auth@two.com', password: 'authtwo2', nameFirst: 'auth', nameLast: 'two' }, timeout: TIMEOUT_MS });
        const token2 = JSON.parse(resUser2.body.toString());
        const res1 = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId}/question/0/duplicate`,
          { json: { token: token2.token }, timeout: TIMEOUT_MS });
        expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
        expect(res1.statusCode).toStrictEqual(403);
      });

      describe('After creating questions', () => {
        let questionId1: {questionId: number};
        ...
        test('QuestionId is not valid', () => {
          const res1 = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId}/question/${questionId1.questionId + 1}/duplicate`,
            { json: { token: token.token }, timeout: TIMEOUT_MS });
          expect(JSON.parse(res1.body.toString())).toStrictEqual(ERROR);
        ...
        test('Returns correct output and timeLastEdited', () => {
          const res1 = request('POST', SERVER_URL + `/v1/admin/quiz/${quiz1.quizId}/question/${questionId1.questionId}/duplicate`,
            { json: { token: token.token }, timeout: TIMEOUT_MS });
          expect(JSON.parse(res1.body.toString())).toStrictEqual({ newQuestionId: expect.any(Number) });
          const time = timeNow();
