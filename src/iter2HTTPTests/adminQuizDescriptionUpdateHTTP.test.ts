...
describe('POST /v1/admin/quiz/:quizid/description', () => {
  ...
  describe('error testing', () => {
    test('returns an error for a description that is too long', () => {
      ...
      const res = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}/description`, {
        json: {
          token: token.token,
    ...
    test('returns an error for invalid quizId', () => {
      const res = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz.quizId + 1}/description`, {
        json: {
          token: token.token,
          description: 'Norman\'s quiz'
        },
    ...
    test('returns an error for a quiz not owned by this user', () => {
      ...
      const res = request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz2.quizId}/description`, {
        json: {
          token: token.token,
          description: 'Norman\'s quiz'
        },
  ...
  describe('functionality testing', () => {
    ...
    test('successfully updates the description of a quiz and the timeLastEdited', () => {
      request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}/description`, {
        json: {
          token: token.token,
          description: 'Norman\'s quiz'
        },
    ...
    test('successfully updates the dscription of multiple quizzes', () => {
      ...
      request('PUT', SERVER_URL + `/v1/admin/quiz/${quiz2.quizId}/description`, {
        json: {
          token: token.token,
      ...
      const res1 = request('GET', SERVER_URL + `/v1/admin/quiz/${quiz.quizId}`, {
        qs: {
          token: token.token
        },
        timeout: TIMEOUT_MS
      ...
      const res2 = request('GET', SERVER_URL + `/v1/admin/quiz/${quiz2.quizId}`, {
        qs: {
          token: token.token
        },
        timeout: TIMEOUT_MS
